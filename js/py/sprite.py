"""`from sprite import *` - sprites for builtin-PyAngelo sketches.

Carried across from the fork's src/lib/sprite.py essentially unchanged, because
it is already plain Python. It has **no imports at all**: every drawing call it
makes - loadImage, drawImage, measureText, saveState, fill, text, rect, circle,
ellipse, stroke, strokeWeight, noStroke, restoreState - is a *builtin* that
js/py/pyangelo_builtins.py installs when a program opens a canvas with
setCanvasSize().

That is why this could not be ported alongside the other classroom modules: it
sits on an API that did not exist yet.
"""
