...
**Everything interpreter-specific about stepping lives in the backend.** `runtime-skulpt.js` owns the `handlers` map (`handlers["*"]` for the stop check, `Sk.debug`/`Sk.delay` for stepping), the suspension-chain walk that finds the deepest `<stdin>.py` frame, the `$loc`/`$tmps` scrape, and stripping Skulpt's `_$rw$` reserved-word suffix. `editor.js` sees only `onStep({lineno, locals})` and returns a promise resolved by the Next button (or a 1s delay under `?autostep`); `populateTraceTable` takes a plain `[[name, value], …]` array. The Pyodide backend will build that same array from `frame.f_locals`.
...
PEP 669's `JUMP` event is the exact equivalent and far cheaper than Skulpt's, which cost a macrotask per iteration. Forward jumps return `DISABLE` so they are never reported again; back-edges yield to the browser (at most once per animation frame) and check the Stop flag. `set_local_events` scopes it to the program's own code objects, so the stdlib and `csinsc` run at full speed.
...
Two bugs worth knowing about, both found by pixel comparison rather than inspection:
